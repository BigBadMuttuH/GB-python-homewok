from random import randint
from timeit import default_timer as timer
import functools


def measure_time(func, currently_evaluating=set()):
    """ декоратор для подсчета времени выполнения функции """

    @functools.wraps(func)
    def inner(*args, **kwargs):
        if func in currently_evaluating:
            return func(*args, **kwargs)
        else:
            start_timer = timer()
            currently_evaluating.add(func)
            try:
                result = func(*args, **kwargs)
            finally:
                currently_evaluating.remove(func)
            end_timer = timer()
            print(f"Function {func.__name__}"
                  f"took {end_timer - start_timer} for execution")
            return result

    return inner


@measure_time
def simple_iteration_sequence(data: list):
    """Поиск последовательности значений простым перебором. O(n^2)"""
    cash = []
    result = []
    if not data:
        return data
    else:
        for i in range(0, len(data)):
            cash.append(data[i])
            for j in range(i + 1, len(data)):
                # Если текущий элемент меньше элемента справа по j
                # и меньше последнего в кэше, то добавляем его в список
                # min_value = min(data[j:])
                # print(f"{data[i]}<{data[j]} and {cash[-1]} < {data[j]}, {min_value=}")
                if data[i] < data[j] and cash[-1] < data[j]:
                    cash.append(data[j])
            # print(f"{cash=}")
            if len(result) < len(cash):
                result = cash[:]
            cash.clear()
    return result


@measure_time
def subsequence(seq):
    """https://ask-dev.ru/info/178880/longest-increasing-subsequence
    вариант с и-нета, для сравнения скорости"""
    if not seq:
        return seq

    M = [None] * len(seq)    # offset by 1 (j -> j-1)
    P = [None] * len(seq)

    # Since we have at least one element in our list, we can start by
    # knowing that the there at least an increasing subsequence of length one:
    # the first element.
    L = 1
    M[0] = 0

    # Looping over the sequence starting from the second element
    for i in range(1, len(seq)):
        # Binary search: we want the largest j <= L
        #  such that seq[M[j]] < seq[i] (default j = 0),
        #  hence we want the lower bound at the end of the search process.
        lower = 0
        upper = L

        # Since the binary search will not look at the upper bound value,
        # we'll have to check that manually
        if seq[M[upper-1]] < seq[i]:
            j = upper

        else:
            # actual binary search loop
            while upper - lower > 1:
                mid = (upper + lower) // 2
                if seq[M[mid-1]] < seq[i]:
                    lower = mid
                else:
                    upper = mid

            j = lower    # this will also set the default value to 0

        P[i] = M[j-1]

        if j == L or seq[i] < seq[M[j]]:
            M[j] = i
            L = max(L, j+1)

    # Building the result: [seq[M[L-1]], seq[P[M[L-1]]], seq[P[P[M[L-1]]]], ...]
    result = []
    pos = M[L-1]
    for _ in range(L):
        result.append(seq[pos])
        pos = P[pos]

    return result[::-1]    # reversing

# Создать список, в который попадают числа,
# описывающие возрастающую последовательность
# и содержащие максимальное количество элементов.
# Дан список чисел.
list_of_data = [randint(-10, 10) for _ in range(0, 10)]
print(list_of_data)
print(f"Наибольшая последовательность чисел:\n"
      f"{simple_iteration_sequence(list_of_data)}")

print(f"Наибольшая последовательность чисел:\n"
      f"{subsequence(list_of_data)}")

